...
export async function POST(req: Request) {
  try {
    ...
    // Run OCR with model waterfall
    let ocrResult: Record<string, unknown> | null = null;
    let lastError: unknown;

    for (const modelName of MODELS) {
      try {
        ocrResult = await runOCR(modelName, image, mimeType);
        break;
      } catch (err: unknown) {
        lastError = err;
    ...
    if (!ocrResult) throw lastError;

    // Clean up expired rows before inserting
    await supabaseAdmin
      .from("esp32_captures")
      .delete()
      .lt("expires_at", new Date().toISOString());

    // Store the capture so the browser poll can pick it up
    const { data, error } = await supabaseAdmin
      .from("esp32_captures")
      .insert({ image_base64: image, mime_type: mimeType, ocr_result: ocrResult })
      .select("id")
      .single();

    if (error) {
      console.error("[esp32-capture] Supabase insert error:", error);
      return Response.json({ error: "Failed to store capture" }, { status: 500 });
    }
